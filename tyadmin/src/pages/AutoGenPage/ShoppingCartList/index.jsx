...
import { addShoppingCart, queryShoppingCart, removeShoppingCart, updateShoppingCart, queryShoppingCartVerboseName, queryShoppingCartListDisplay, queryShoppingCartDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryUserProfile, queryUserProfileVerboseName} from '@/pages/AutoGenPage/UserProfileList/service';import {queryGoods, queryGoodsVerboseName} from '@/pages/AutoGenPage/GoodsList/service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateShoppingCart(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryShoppingCartDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryShoppingCartListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryShoppingCart, table_columns, '购物车-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索购物车" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '购物车-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryShoppingCart({ ...params, sorter, filter })}
